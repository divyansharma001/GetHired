...
import { Plus, FileText, Download, Calendar, Star, MoreVertical, Sparkles, Sun, Moon, Briefcase, BarChart3, DownloadCloud, Loader2, Trash2, Edit3, AlertTriangle } from 'lucide-react'; // Added Loader2, Trash2, Edit3
import { useTheme } from '@/context/theme-provider';
import { useResumeStore } from '@/hooks/use-resume'; // For resetting store when creating new

// Define the Resume type for data fetched from API
// This should match what your GET /api/resumes returns
interface ResumeFromApi {
  id: string;
  title: string;
  atsScore: number;
  updatedAt: string; // This will be a string from JSON, needs new Date() for formatting
  status: 'draft' | 'completed'; // As derived or fetched
}
...
const DashboardContent: React.FC<DashboardContentProps> = ({ userId }) => {
  ...
  const resetResumeInStore = useResumeStore(state => state.resetResume); // For "Create New"

  const isDark = theme === 'dark';
  const themeClasses = { /* ... (keep your existing themeClasses) ... */ 
  ...
  }, []); // No dependencies needed if it's only called once or manually refreshed

  useEffect(() => {
    fetchResumes();
  }, [fetchResumes]); // Fetch on component mount

  const handleCreateNewClick = () => {
    // Reset the Zustand store to a fresh state for a new resume
    if (userId) {
  ...
  const handleDeleteResume = async (resumeId: string) => {
    ...
    try {
      ...
      // Refetch resumes or filter out the deleted one locally
      setResumes(prevResumes => prevResumes.filter(r => r.id !== resumeId));
      alert('Resume deleted successfully.'); // Replace with toast
    } catch (err) {
      alert(`Error deleting resume: ${err instanceof Error ? err.message : String(err)}`);
    }
  };

  const getScoreColorText = (score: number) => { /* ... (keep as is) ... */ 
  ...
  const getScoreBgClass = (score: number) => { /* ... (keep as is) ... */ 
        ...
        <div className={`min-h-screen ${themeClasses.bg} flex flex-col`}>
            ...
            <div className="flex-grow flex flex-col items-center justify-center text-center px-4">
                ...
                <button 
                    ...
                    className={`${themeClasses.accentGradient} ${themeClasses.accentHover} text-white px-6 py-2.5 rounded-xl font-semibold`}
                >
                    Try Again
                </button>
            </div>
    ...
    <div className={`min-h-screen ${themeClasses.bg} transition-colors duration-300`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {/* ... (Stat card mapping remains the same, ensure values are dynamic if needed) ... */}
          {[
            { title: 'Total Resumes', value: resumes.length, change: `+${resumes.filter(r => new Date(r.updatedAt) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)).length} this month`, icon: statIcons[0] }, // Example dynamic change
            ...
            { title: 'Downloads', value: 0, change: 'Feature pending', icon: statIcons[2] } // Placeholder for downloads
          ].map((stat) => (
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Link href="/dashboard/create-resume" passHref>
            <button 
              ...
              className={`${themeClasses.accentGradient} ${themeClasses.accentHover} text-white px-6 py-3 rounded-xl font-semibold flex items-center transition-all duration-200 shadow-lg hover:shadow-purple-500/30 transform hover:-translate-y-0.5`}>
        ...
        {/* Resume List - Replaces loading state if not loading and no error */}
        {resumes.length === 0 ? (
           <div className={`${themeClasses.cardBg} text-center py-16 px-6 rounded-xl border ${themeClasses.border} shadow-xl`}>
            ...
            <Link href="/dashboard/create-resume" passHref>
              <button 
                onClick={handleCreateNewClick}
                className={`${themeClasses.accentGradient} ${themeClasses.accentHover} text-white px-8 py-3.5 rounded-xl font-semibold text-lg`}>
              ...
              <div key={resume.id} className={`${themeClasses.cardBg} rounded-xl border ${themeClasses.border} ${themeClasses.cardBorderHover} transition-all duration-300 shadow-lg hover:shadow-2xl flex flex-col`}>
                <div className="p-6 flex-grow">
                  <div className="flex items-start justify-between mb-3">
                    <h4 className={`text-lg font-semibold ${themeClasses.text} group-hover:text-purple-400 transition-colors truncate`}> {/* Added truncate */}
                      {resume.title}
                    </h4>
                    <button 
                        onClick={() => handleDeleteResume(resume.id)}
                        title="Delete Resume"
                        className={`p-1.5 rounded ${themeClasses.buttonGhost} text-red-500/70 hover:text-red-500 hover:bg-red-500/10`}
                    >
                ...
                <div className={`border-t ${themeClasses.border} p-4 flex space-x-3`}>
                  {/* TODO: Update Link href for editing when edit route is ready */}
                  <Link href={`/dashboard/create-resume?resumeId=${resume.id}`} className="flex-1"> 
                    <button className={`w-full px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center ${themeClasses.text} ${themeClasses.buttonOutline} transition-colors`}>
                      <Edit3 className="w-4 h-4 mr-2" /> {/* Changed icon */}
                      Edit
                    </button>
                  </Link>
                  <button 
                    title="Download PDF (coming soon)"
                    disabled // Temporarily disable download
                    className={`px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-center ${themeClasses.text} ${themeClasses.buttonOutline} transition-colors disabled:opacity-50 disabled:cursor-not-allowed`}>
